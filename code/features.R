...
# Cases where winner is higher ranked but lower seed
aggdata %>% filter(winner_rank_points > loser_rank_points, winner_seed > loser_seed) %>% select(ends_with("_rank_points"), ends_with("seed"), ends_with("name")) %>% View()

# Cases where higher ranked player is lower seed
aggdata %>% filter(winner_rank_points > loser_rank_points, winner_seed > loser_seed) %>% select(ends_with("_rank_points"), ends_with("seed"), ends_with("name")) %>% nrow()
aggdata %>% filter(winner_rank_points < loser_rank_points, winner_seed < loser_seed) %>% select(ends_with("_rank_points"), ends_with("seed"), ends_with("name")) %>% nrow()


hist(aggdata$winner_rank_points - aggdata$loser_rank_points)

# Designate each match as a win or loss
outcomedata <- aggdata %>%
  mutate(win = ifelse(winner_seed > loser_seed))

splitdata <- split(outcomedata, outcomedata$win)

# Apply randomization and recombine dataset
# Add a few features
modeldset <- map(splitdata, prep_dset)

modeldset <- union_all(modeldset[[1]], modeldset[[2]]) 
